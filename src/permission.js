...
router.beforeEach((to, from, next) => {
    ...
    // 1.1 如果没有获取到，要访问非登录页面，则不让访问，跳到登录页面 /login
    if (!token) {
        if (to.path !== '/login') {
            next({ path: '/login' })
        } else {
        ...
        if (to.path === '/login') {
            ...
            const userInfo = localStorage.getItem('mxg-msm-uese')
            if (userInfo) {
                // 本地获取到，则直接去目标路由
                next()
            } else {
                ...
                getUserInfo(token).then((res) => {
                    const resp = res.data;
                    if (resp.flag) {
                        // 如果本地有用户信息，则进行非登录页面，否则回到登录页面
                        // 保存到本地
                        localStorage.setItem('mxg-msm-uese', JSON.stringify(resp.data))
                        next()
                    } else {
                        // 未获取到用户信息，重新登录
                        next({ path: '/login' })
